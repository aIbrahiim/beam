...
class TestPipeline(Pipeline):
  ...
  def get_options_list(cls, argv=None):
    """Parse argv (default sys.argv) for --test-pipeline-options and return
    the list of pipeline option strings. Used by load tests that need to
    create the pipeline with a custom options class."""
    parser = argparse.ArgumentParser()
    parser.add_argument(
        '--test-pipeline-options',
        type=str,
