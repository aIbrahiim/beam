...
class TableRowModelHandler(SklearnModelHandlerNumpy):
  ...
  def run_inference(
    ...
    features_array = []
    for row in batch:
      features = [getattr(row, col) for col in self.feature_columns]
...
def parse_json_to_table_row(
    message: bytes, schema_fields: Optional[list[str]] = None
) -> tuple[str, beam.Row]:
...
def run(
  ...
  if known_args.mode == 'streaming' and not known_args.input_subscription:
    raise ValueError(
        'input_subscription is required for streaming mode')
